...
function getHintBtns() {
  var texts = ['₿ BTC 价格','Ξ ETH 费率','📊 市场情绪','📈 RSI 分析','🔥 热门币种','💡 交易机会'];
  var qs = ['BTC 现在什么价格？','ETH 资金费率多少？','市场情绪怎么样？','BTC RSI 超买还是超卖？','最近有什么热门币？','帮我分析一下现在的交易机会'];
  return texts.map(function(t,i){ return '<button onclick=\"quickAsk(\\''+qs[i]+'\\')\">'+t+'</button>'; }).join('');
}
...
try {
  ...
  console.log('✅ Clean JS syntax OK');
} catch(e) {
  console.log('❌ Clean JS syntax FAIL:', e.stderr?.toString()?.slice(0,200));
  process.exit(1);
}
...
console.log('✅ chat-app/index.html rewritten');
