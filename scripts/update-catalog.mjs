...
const endMark = "_tips:"
// Find the closing of CATALOG - it's the `}` before the next `_tips` usage in the handler
const end = src.indexOf("\n  }", src.indexOf(endMark, start))
if (start < 0 || end < 0) {
  console.log("ERROR: could not find CATALOG block")
  process.exit(1)
}
// Also need to find the actual end - the CATALOG object ends with `}\n\n\t  //` or `}\n  }`
// Let's find the } that closes `const CATALOG = {`
let depth = 0
let catalogEnd = -1
for (let i = start + "const CATALOG = ".length; i < src.length; i++) {
  if (src[i] === "{") depth++
  if (src[i] === "}") { depth--; if (depth === 0) { catalogEnd = i + 1; break } }
}
if (catalogEnd < 0) {
  console.log("ERROR: could not find CATALOG closing bracket")
  process.exit(1)
}

src = src.slice(0, start) + newBlock + src.slice(catalogEnd)
writeFileSync("src/tools/agent-utils.ts", src, "utf8")
console.log("CATALOG replaced successfully")
console.log("Public domains (authRequired:false):", newBlock.match(/authRequired: false/g)?.length || 0)
console.log("Auth domains (authRequired:true):", newBlock.match(/authRequired: true/g)?.length || 0)
