...
export function registerCodeGraphTools(server: McpServer): void {
  ...
  registerTool(
    ...
    "code_status",
    "READ",
    "[D:CodeIntel] 代码知识图谱状态：节点数/边数/覆盖文件/被调Top函数 | 无需参数 | 查调用链用 code_query",
    {},
    async () => {
      try {
  ...
  registerTool(
    ...
    "code_query",
    "READ",
    "[D:CodeIntel] 查询代码知识图谱：调用链(callers/callees)/符号搜索/影响分析 | mode, symbol?, limit?默认15 | 先 code_status 确认图谱就绪",
    {
