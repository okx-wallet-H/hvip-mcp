        ...
        for (const ind of list) {
          ...
          try {
            switch (ind) {
              ...
              case "adx": { const ax = calcADX(candles, p); const a = ax.adx[ax.adx.length - 1]; const pd = ax.plusDI[ax.plusDI.length - 1]; const md = ax.minusDI[ax.minusDI.length - 1]; item.value = `${a.toFixed(1)}`; const sig = adxSignal(a, pd, md); item.signal = sig.signal; item.message = sig.message; break }
              case "mfi": { const mf = calcMFI(candles, p); item.value = mf[mf.length - 1].toFixed(1); item.signal = mf[mf.length - 1] > 80 ? "bearish" : mf[mf.length - 1] < 20 ? "bullish" : "neutral"; item.message = `MFI=${item.value}`; break }
              case "cmf": { const cm = calcCMF(candles, p); item.value = cm[cm.length - 1].toFixed(3); item.signal = cm[cm.length - 1] > 0 ? "bullish" : "bearish"; item.message = `CMF=${item.value}`; break }
              case "supertrend": { const st = calcSupertrend(candles, p, 3); item.value = st.trend[st.trend.length - 1].toFixed(2); item.signal = st.direction[st.direction.length - 1] === 1 ? "bullish" : "bearish"; item.message = `${item.signal === "bullish" ? "多头" : "空头"}`; break }
